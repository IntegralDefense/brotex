...
export {
    ...
    const stream_dir = "/var/streams" &redef;
    ...
    const stream_ready_dir = "/var/streams_ready" &redef;
}

# returns T if the given mime type should be excluded or F otherwise
function is_excluded_mimetype(mime_type:string): bool {

    # is this even a mime type?
    if (/^[^\/]+\/[^\/]+$/ != mime_type) 
        return F;

    # we don't scan any of these
    ...
    if (mime_type == "application/x-fcs")
        return T;

    if (mime_type == "application/json")
        return T;

    if (mime_type == "application/binary")
        return T;
    ...
    if (mime_type == "application/vnd.ms-fontobject")
        return T;

    if (mime_type == "text/troff")
        return T;
    ...
    # we don't scan images, sounds and video
...
function record_smtp_stream(c: connection):bool {
    ...
    return c$id$resp_p == 25/tcp;
}

function record_http_stream(c: connection):bool {
    # returns T if this connection is an HTTP session we want
    #print(fmt("internal %s proxy %s", c$id$orig_h, c$id$resp_h));
    #return is_internal(c$id$orig_h) && is_proxy(c$id$resp_h);
    #return (is_proxy(c$id$orig_h) || is_internal(c$id$orig_h)) && ! is_internal(c$id$resp_h);
...
event check_file_chunk(f: fa_file, data: string, off:count) {
    ...
    if (sub_bytes(data, 0, 3) == "ID3") {
        #print(fmt("detected ID3 audio in %s", f$info$filename));

        # blacklist this file
        f$info$blacklisted = T;

        # stop the analyzer
        Files::remove_analyzer(f, Files::ANALYZER_EXTRACT, [$extract_filename=f$info$extraction_path]);
        Files::remove_analyzer(f, Files::ANALYZER_DATA_EVENT, [$chunk_event=check_file_chunk]);

        # decrement the extraction count for the session
        for (cid in f$conns) {
            local c: connection = f$conns[cid];
...
function file_new_http(c: connection, f: fa_file):bool {
    ...
    # make a decision here if we are going to extract this file from this http stream
    #print(fmt("file_new http event: %s %s %s", c$uid, f$info, c$http));

    local extract:bool = T;

    # do we have a mime type?
    local mime_type:string = "";
    if (f$info?$mime_type)
        mime_type = f$info$mime_type;
    else if (c$http$current_mime_type != "")
        mime_type = c$http$current_mime_type;

    # check the mime type as reported by the web server
    # note that this can be spoofed
    if (c$http$current_mime_type != "")
        if (is_excluded_mimetype(c$http$current_mime_type)) {
            #print(fmt("excluding http mime_type %s", c$http$current_mime_type));
            extract = F;
        }

    # trust the mime_type value first
    # assuming mime type property of file is from content
    if (f$info?$mime_type) {
        #print(fmt("file mime_type = %s", f$info$mime_type));

        if (is_excluded_mimetype(f$info$mime_type)) {
            #print(fmt("excluding file mime_type %s", f$info$mime_type));
            extract = F; # this can change if the file extension is something we want to see
        }
    }
    ...
    if (! f$info?$filename) {
        if (mime_type != "") {
            # use the fuid and the extension for the mime type
            if (mime_type in brotex::mime_type_to_file_extension_map) {
                ...
                #print(fmt("got file name %s from mime type", f$info$filename));
            }
        }

        # no mime type but does the URI end in something that looks like a file extension?
        if (c$http?$uri && c$http$uri == /^.*\/[^\/\?]+\.[a-zA-Z0-9_]{3}$/) {
            f$info$filename = find_last(c$http$uri, /\/[^\/\?]+\.[a-zA-Z0-9_]{3}$/);
            f$info$filename = f$info$filename[1:];
            #print(fmt("found file name %s from the URI %s", f$info$filename, c$http$uri));
        }
    }

    # we have a chance here to extract regardless of mime type if the 
    # determined "file extension" is something interesting
    if (! extract && f$info?$filename && |f$info$filename| > 0) {
        # ok what about based on file ext
        local ext = find_last(f$info$filename, /\.[a-zA-Z0-9_]{3}$/);
        if (ext != "") {
            ext = ext[1:];
            # is this a file extension we want to extract?
            if (ext in brotex::extracted_file_extensions) {
                #if (! extract)
                    #print(fmt("forced extracting %s based on file extension %s", f$info$filename, ext));
                extract = T;
            }
        }
    }
    ...
    #print(fmt("accepted mime_type %s", mime_type));
    ...
    # inspect the first chucnk of the file to look for html/javascript
